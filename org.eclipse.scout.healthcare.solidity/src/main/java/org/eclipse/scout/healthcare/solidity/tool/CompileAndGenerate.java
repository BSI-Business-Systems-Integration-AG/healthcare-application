package org.eclipse.scout.healthcare.solidity.tool;
...
public class CompileAndGenerate {
	...
	public static final String BASE_PACKAGE = "org.eclipse.scout.healthcare.solidity";
	
	public static final String FOLDER_SOURCE = "\\src\\main\\resources";
	public static final String FOLDER_TARGET = "\\src\\main\\java";
	...
	public static void main(String argv []) throws Exception {
		if (argv.length == 2) {
			String packagePath = argv[0];
			String contract = argv[1];
			CompileAndGenerate cag = new CompileAndGenerate(packagePath, contract);
			cag.run();
		} else {
			System.out.printf("Not enough or too many argument! Expected: {package_path} {contract_name}");
		}
	}

	public CompileAndGenerate(String packagePath, String contract) {
	...
	public void generateJava() throws Exception {
		String binaryFile = getBinaryFileName(m_contract, m_packagePath + FOLDER_SOURCE);
		String abiFile = getAbiFileName(m_contract, m_packagePath + FOLDER_SOURCE);
		String [] cmdLine = {binaryFile, abiFile, "-p", BASE_PACKAGE, "-o", m_packagePath + FOLDER_TARGET};
	...
	public void compileSolidity() throws Exception {
		String sourceCode = readSolidity(String.format("%s\\%s.%s", m_packagePath + FOLDER_SOURCE, m_contract, EXT_SOLIDITY));
		@SuppressWarnings("unused")
		JsonObject response = compileSolidity(sourceCode, m_contract, m_packagePath + FOLDER_SOURCE);		
	}
